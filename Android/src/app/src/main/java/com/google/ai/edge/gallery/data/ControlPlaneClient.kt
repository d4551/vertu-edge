...
class HttpControlPlaneClient(
  ...
  override suspend fun fetchProviderModels(
    ...
    withContext(Dispatchers.IO) {
      ...
      val statePair = parseStateMessage(response = response, stateId = stateId)
      val models = parseOptionValues(response = response)

      CloudModelOptionsResult(
        models = models.distinct(),
        selectedModel = extractSelectedOption(response),
  ...
  private fun <T : Any> parseEnvelope(
    ...
    val rawJson = parseEnvelopeBody(response)
    val envelopeType = TypeToken.getParameterized(CloudControlPlaneEnvelope::class.java, type).type
    return try {
      gson.fromJson(rawJson, envelopeType)
    } catch (error: JsonSyntaxException) {
      throw CloudControlPlaneError.EnvelopeDecodeFailed(error.message ?: "Failed to decode envelope", error)
    }
  }

  private fun parseEnvelopeBody(response: String): String {
    val trimmed = response.trim()
    if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
      return trimmed
    }
    val matcher =
      DATA_ENVELOPE_PATTERN.matcher(response)
  ...
  private fun parseStateMessage(response: String, stateId: String): Pair<FlowExecutionState, String> {
    ...
    val rawMessage = stripHtml(response = stateMatcher.group(2) ?: "").trim()
    return toFlowExecutionState(rawState) to
      if (rawMessage.isNotBlank()) rawMessage else ""
  }

  private fun parseOptionValues(response: String): List<String> {
    val matches =
      OPTION_PATTERN.matcher(response)
    val values = mutableListOf<String>()
    while (matches.find()) {
      val value = matches.group(1)
      val decoded = unescapeHtmlEntities(value)
  ...
  private fun extractSelectedOption(response: String): String? {
    val matches =
      SELECTED_OPTION_PATTERN.matcher(response)
    if (matches.find()) {
      return unescapeHtmlEntities(matches.group(1))
    }
    return null
  }
  ...
  private fun executePostRequest(urlString: String, payload: String): String {
    var connection: HttpURLConnection? = null
    try {
      val url = parseUrl(urlString)
      connection = (url.openConnection() as HttpURLConnection).apply {
